	...
	for (var j = availableOptions.length - 1; j >= 0; j--) {
		if (availableOptions[j].length > 0 ) {
			for (var j2 = availableOptions[j].length - 1; j2 >= 0; j2--) {
				...
				var buttonElem = $(parentElem).find(".product-form__swatch__item[data-value='" + availableOptions[j][j2] + "']");
...
theme.regionCheckerCookie = function() {
  var regionCookie = "region-checked";
  
  
  
    var status = getParameterByName('pop');
  
  if (typeof Cookies != "undefined") {
	  if (Cookies(regionCookie) !== "1") {
// window.onload = function(e){  
document.querySelector(".area-checker").style.display="block";
  
//     if (status == 'false'){
// 	$.magnificPopup.close();
//         document.querySelector(".area-checker").style.display="none";
  
//     }
// }
      }
  }
  $(".area-checker .c-btn--default").on('click', function(e) {
    console.log(rr);
		Cookies(regionCookie, "1", { expires: 14, path: "/" });
		$.magnificPopup.close();
